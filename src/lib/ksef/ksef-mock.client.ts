...
const MOCK_PRODUCTS_COST = [
  { desc: "Opakowania kartonowe 500ml (paleta)", unit: "szt", price: 1200 },
  { desc: "Cukier biały (tona)", unit: "t", price: 3200 },
  { desc: "Nawóz organiczny BioGrow 25kg", unit: "szt", price: 89 },
  { desc: "Transport chłodniczy Zakopane-Warszawa", unit: "kurs", price: 2800 },
  { desc: "Etykiety samoprzylepne (rolka 5000szt)", unit: "szt", price: 180 },
  { desc: "Folia termokurczliwa 200m", unit: "szt", price: 150 },
  { desc: "Słoiki szklane 350ml (paleta 500szt)", unit: "szt", price: 950 },
];

const MOCK_PRODUCTS_SALES = [
  { desc: "Żelki gumijagodowe premium 200g (karton 48szt)", unit: "karton", price: 180 },
  { desc: "Syrop gumijagodowy 500ml (karton 12szt)", unit: "karton", price: 150 },
  { desc: "Konfitura gumijagodowa 300g (karton 24szt)", unit: "karton", price: 220 },
  { desc: "Żelki gumijagodowe classic 100g (karton 96szt)", unit: "karton", price: 240 },
  { desc: "Mix prezentowy gumijagodowy (karton 12szt)", unit: "karton", price: 360 },
];
  ...
  const lineItemsXml = invoice.lineItems
    .map(
      ...
      <FaWiersz>
        ...
        <P_9A>${item.unitPriceNet.toFixed(2)}</P_9A>
        <P_11>${item.amountNet.toFixed(2)}</P_11>
...
<Faktura xmlns="http://crd.gov.pl/wzor/2023/06/29/12648/">
  ...
  <Fa>
    ...
    <FaWiersze>
      ...
      <WartoscWierszyFaktur1>${invoice.amountNet.toFixed(2)}</WartoscWierszyFaktur1>
      ${lineItemsXml}
    </FaWiersze>
    <Podsumowanie>
      <P_13_1>${invoice.amountNet.toFixed(2)}</P_13_1>
      <P_14_1>${invoice.amountVat.toFixed(2)}</P_14_1>
    </Podsumowanie>
    <Platnosc>
...
export class MockKSeFClient implements IKSeFClient {
  ...
  async fetchInvoices(params: KSeFFetchParams): Promise<KSeFInvoice[]> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const totalVat = Math.round(totalNet * vatRate * 100) / 100;
      ...
      const invoiceData = {
        ...
        bankAccountNumber: `${randomInt(10, 99)}1090${randomInt(1000, 9999)}0000${randomInt(1000000000, 9999999999)}${randomInt(1000, 9999)}`,
      };
